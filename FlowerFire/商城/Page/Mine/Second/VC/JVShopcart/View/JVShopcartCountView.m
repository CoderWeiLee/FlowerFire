...
- (UITextField *)editTextField {
    if(_editTextField == nil) {
        ...
        _editTextField.layer.borderColor = rgba(204, 204, 204, 1).CGColor;
        _editTextField.layer.borderWidth = 0.5;
        _editTextField.textColor = rgba(51, 51, 51, 1);
...
- (void)layoutSubviews {
    ...
    [self.editTextField mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.decreaseButton.mas_right).offset(-1);
        make.right.equalTo(self.increaseButton.mas_left).offset(1);
    }];
}
