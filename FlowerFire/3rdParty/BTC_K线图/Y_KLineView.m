...
- (Y_StockChartRightYView *)priceView
{
    if(!_priceView)
    {
        ...
        _priceView.precision = 2;//self.kLineModels.firstObject.price;
...
- (Y_StockChartRightYView *)volumeView
{
    if(!_volumeView)
    {
        ...
        _volumeView.precision = 6;//self.kLineModels.firstObject.coin;
...
- (void)event_longPressMethod:(UILongPressGestureRecognizer *)longPress {
    ...
    if(UIGestureRecognizerStateChanged == longPress.state || UIGestureRecognizerStateBegan == longPress.state){
       ...
       //     self.verticalView.backgroundColor = gTextColorSub;
        ...
        NSString *dateStr;
        if(NSStringFormat(@"%@",self.currentKLineModel.Date).length != 19){
            //不除以1000
            NSDate *date = [NSDate dateWithTimeIntervalSince1970:(self.currentKLineModel.Date.doubleValue) / 1];
            ...
            if (self.lineKTime == 0 || self.lineKTime == 4) {
             ...
             dateStr = self.currentKLineModel.Date;
        }
        ...
        //手指在屏幕上的x坐标
        CGPoint windowLocation = [longPress locationInView:[UIApplication sharedApplication].delegate.window];
        if (self.isFullScreen){ //横屏
            if (windowLocation.x > ScreenWidth/2.0) {
                pricePositionX = self.scrollView.contentOffset.x + ScreenWidth - 55-88-SafeAreaBottomHeight ; //减去指标宽度 右侧价格宽度
            }else {
                pricePositionX = self.scrollView.contentOffset.x + SafeAreaTopHeight; //减去指标宽度 右侧价格宽度; //减去指标宽度
            }
        }else{
            if (windowLocation.x > ScreenWidth/2.0) {
                pricePositionX = self.scrollView.contentOffset.x + ScreenWidth - 60;
            }else {
        ...
        if (self.isFullScreen){
            [self.tipBoard showWithTipPoint:CGPointMake(windowLocation.x, 20) isFullScreen:YES];
        }else{
...
- (BTKLineTipBoardView *)tipBoard {
    if (!_tipBoard) {
        ...
        _tipBoard.backgroundColor = [UIColor whiteColor];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"scrollViewDidScrollNotice" object:nil userInfo:@{@"isScroll":@"1"}];
}
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"scrollViewDidScrollNotice" object:nil userInfo:@{@"isScroll":@"0"}];
}
