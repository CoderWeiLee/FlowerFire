...
- (CAShapeLayer *)currentPriceLayer {
    if (!_currentPriceLayer) {
        ...
        [_currentPriceView setUserInteractionEnabled:YES];
        @weakify(self)
        [_currentPriceView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithActionBlock:^(id  _Nonnull sender) {
            @strongify(self)
            [self.parentScrollView setContentOffset:CGPointMake(self.parentScrollView.contentSize.width - self.parentScrollView.width, 0) animated:YES];
        }]];
...
- (void)drawBottomDateValue {
    ...
    [self.needDrawKLinePositionModels enumerateObjectsUsingBlock:^(Y_KLinePositionModel * _Nonnull positionModel, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        NSString *dateStr;
        if(NSStringFormat(@"%@",self.needDrawKLineModels[idx].Date).length != 19){
            //日期  不除以1000
            NSDate *date = [NSDate dateWithTimeIntervalSince1970:(self.needDrawKLineModels[idx].Date.doubleValue)/1];
            ...
            dateStr = self.needDrawKLineModels[idx].Date;
        }
        ...
        if(CGPointEqualToPoint(lastDrawDatePoint, CGPointZero) || (point.x - lastDrawDatePoint.x > 60) ){
            ...
            if (self.isFullScreen) {
                CGRect rect = CGRectMake(drawDatePoint.x + 2, ScreenHeight - 60 - 18+2, 50, self.frame.size.height -Y_StockChartKLineMainViewMaxY);
            ...
            //        y = 345;
             //   }
                //视图高度 - 自身高度20
                CGRect rect = CGRectMake(drawDatePoint.x + 2, 350 - 18, 50, self.frame.size.height -Y_StockChartKLineMainViewMaxY);
                CATextLayer * textLayer = [CATextLayer fl_getTextLayerWithString:dateStr textColor:[UIColor assistTextColor] fontSize:9.f backgroundColor:[UIColor clearColor] frame:rect];
                lastDrawDatePoint = drawDatePoint;
                [self.dateLayer addSublayer:textLayer];
            }
...
- (void)drawMaxAndMinValueAndCurrentPrice{
    ...
    CGFloat priceRectx = lastPointX.x;
     CGFloat screenWidth;
    if(((AppDelegate *)[UIApplication sharedApplication].delegate).isEable){
        if(SCREEN_WIDTH>=812.0f){
            screenWidth = SCREEN_WIDTH - 60 - 21;
        }else{
