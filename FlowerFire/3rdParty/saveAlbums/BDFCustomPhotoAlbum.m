...
- (void)saveimageIntoAlbum:(nonnull UIImage *)image  {
    ...
    if (createdAssets == nil) {
        // NSLog(@"保存图片失败");
        [self showAlertMessage:LocalizationKey(@"Failed to save image")];
    }
    // 2.拥有一个【自定义相册】
    PHAssetCollection * assetCollection = self.createCollection;
    if (assetCollection == nil) {
        // NSLog(@"创建相册失败");
        [self showAlertMessage:LocalizationKey(@"Failed to create album")];
    }
    ...
    if (error) {
        ...
        // NSLog(@"保存图片成功");
        [self showAlertMessage:LocalizationKey(@"Save the picture successfully")];
    }
}
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (status == PHAuthorizationStatusDenied ) { // 用户拒绝当前App访问权限
                if (oldStatus != PHAuthorizationStatusNotDetermined) {
                    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:LocalizationKey(@"Photo permissions are not turned on") message:LocalizationKey(@"PhotoPermissionip") preferredStyle:UIAlertControllerStyleAlert];
                    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:LocalizationKey(@"Not yet") style:UIAlertActionStyleCancel handler:nil];
                    UIAlertAction *okAction = [UIAlertAction actionWithTitle:LocalizationKey(@"Go to set") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                ...
                printAlert(LocalizationKey(@"photoErrorTip"), 1.f);
            }
        });
    }];
}
...
- (void)showAlertMessage:(NSString *)message {
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:nil
                                         ...
                                         otherButtonTitles:LocalizationKey(@"determine"), nil];
