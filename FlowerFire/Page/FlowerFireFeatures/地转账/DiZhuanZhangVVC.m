...
@interface DiZhuanZhangVVC ()<UITextFieldDelegate>
{
    UIButton *button2;
    LightningChargeFormView *_herIDFormView,*_transNumFormView,*_pwdFormView,*_selectedCoinFormView,*_emailFormView,*_dayview;
    WTLabel *_moneyLabel;
...
@implementation DiZhuanZhangVVC
...
-(void)createNavBar{
    self.gk_navigationItem.title = LocalizationKey(@"578Tip888");
}

- (void)createUI{
    
    _herIDFormView = [[LightningChargeFormView alloc] initWithFrame:CGRectMake(OverAllLeft_OR_RightSpace, 22+Height_NavBar, ScreenWidth - 2 *OverAllLeft_OR_RightSpace, 45) leftText:LocalizationKey(@"578Tip12444444") placeHolderStr:@""];
    _herIDFormView.enabled = NO;
...
//    [button setImage:[UIImage imageNamed:@"jiantouarrow492"] forState:0];
//    button.frame = CGRectMake(_selectedCoinFormView.right - 40,0,25,25);
//    [_selectedCoinFormView addSubview:button];

    
    _transNumFormView = [[LightningChargeFormView alloc] initWithFrame:CGRectMake(OverAllLeft_OR_RightSpace, _selectedCoinFormView.bottom + 22, ScreenWidth - 2 *OverAllLeft_OR_RightSpace, 45) leftText:LocalizationKey(@"575Tip24444444444") placeHolderStr:@""];
    _transNumFormView.keyboardType = UIKeyboardTypeNumberPad;
    
    [self.view addSubview:_transNumFormView];
    
    _moneyLabel = [[WTLabel alloc] initWithFrame:CGRectMake(_transNumFormView.left + 15, _transNumFormView.bottom + 13, ScreenWidth, 20) Text:LocalizationKey(@"575Tip27777777777") Font:tFont(13) textColor:rgba(51, 51, 51, 1) parentView:self.view];
...
//    [self.view addSubview:_selectedCoinFormView];
    ...
    _dayview = [[LightningChargeFormView alloc] initWithFrame:CGRectMake(OverAllLeft_OR_RightSpace, _moneyLabel.bottom + 22, ScreenWidth - 2 *OverAllLeft_OR_RightSpace, 45) leftText:LocalizationKey(@"575Tip28888888888") placeHolderStr:@""];
    ...
    _pwdFormView = [[LightningChargeFormView alloc] initWithFrame:CGRectMake(OverAllLeft_OR_RightSpace, _dayview.bottom + 22, ScreenWidth - 2 *OverAllLeft_OR_RightSpace, 45) leftText:LocalizationKey(@"575Tip28") placeHolderStr:LocalizationKey(@"575Tip29")];
...
- (void)dataNormal:(NSDictionary *)dict type:(NSString *)type{
    if([type isEqualToString:@"1"]){
        ...
        NSMutableAttributedString *ma = [[NSMutableAttributedString alloc] initWithString:NSStringFormat(@"%@ %@",LocalizationKey(@"575Tip27777777777"),useableStr)];
        [ma yy_setColor:rgba(51, 51, 51, 1) range:[[ma string] rangeOfString:LocalizationKey(@"575Tip27777777777")]];
...
//    }
    ...
    md[@"day"] = self->_dayview.text;//_herID; //对方的用户id或者SD钱包地址
   ...
   // md[@"code"] = _emailFormView.text;
    [self.afnetWork jsonPostDict:@"/api/lock/enter" JsonDict:md Tag:@"4"];
}
//币币闪充转账获取货币列表
