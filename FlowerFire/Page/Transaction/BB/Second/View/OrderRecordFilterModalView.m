...
const CGFloat viewHeight = 310;

@interface OrderRecordFilterModalView ()<UICollectionViewDelegate,UICollectionViewDataSource>
{
...
@implementation OrderRecordFilterModalView
...
-(void)cleanClick{//重置数据
    ...
    !self.orderRecordFilterBlock ? : self.orderRecordFilterBlock(self.paramsType,self.paramsStatus);
}
...
-(void)initData{
    self.sectionArray = @[LocalizationKey(@"TransactionType"),LocalizationKey(@"Order Status")];
    self.dataArray = @[@[@{@"btnName":LocalizationKey(@"FiatBuy")},@{@"btnName":LocalizationKey(@"FiatSell")}],
                        @[@{@"btnName":LocalizationKey(@"OrderTip1")},@{@"btnName":LocalizationKey(@"OrderTip2")},@{@"btnName":LocalizationKey(@"ccTip9")},
                          @{@"btnName":LocalizationKey(@"OrderTip3")},@{@"btnName":LocalizationKey(@"OrderTip4")}]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(!cell){
        cell = [[OrderRecordFilterModalCell alloc] initWithFrame:CGRectZero];
    }
...
-(void)setParamsType:(NSString *)paramsType{
    if([paramsType isEqualToString:LocalizationKey(@"FiatBuy")]){
        _paramsType = @"0";
    }else if([paramsType isEqualToString:LocalizationKey(@"FiatSell")]){
        _paramsType = @"1";
    }else{
...
-(void)setParamsStatus:(NSString *)paramsStatus{
    if([paramsStatus isEqualToString:LocalizationKey(@"OrderTip4")]){
        _paramsStatus = @"-2";
    }else if ([paramsStatus isEqualToString:LocalizationKey(@"OrderTip3")]){
        _paramsStatus = @"-1";
    }else if ([paramsStatus isEqualToString:LocalizationKey(@"ccTip9")]){
        _paramsStatus = @"2";
    }else if ([paramsStatus isEqualToString:LocalizationKey(@"OrderTip2")]){
        _paramsStatus = @"1";
    }else if ([paramsStatus isEqualToString:LocalizationKey(@"OrderTip1")]){
        _paramsStatus = @"0";
    }else{
