...
@interface PYHandleDate ()
@property (nonatomic,strong)NSDateFormatter *dateFormatter;
@end

@implementation PYHandleDate
//单利
...
- (NSDate *)returnDateWithOBJ: (NSObject *)date_OBJ andDateFormatter: (NSString *)dateFormatterStr{
    ...
    if ([date_OBJ isKindOfClass:[NSString class]]){//是字符串
        ...
        if (!date){//如果没有转化成功，按NSNumber类型处理
            ...
            date = [self getLocalDateFormatAnyDate:date];
...
//    NSDate *expireDate = [dateFomatter dateFromString:endDateStr];
    ...
    // 对比时间差
    NSDateComponents *dateCom = [calendar components:unit fromDate:startDateTemp toDate:endDateTemp options:NSCalendarWrapComponents ];
...
//    int day = (int)value / (24 * 3600);
    ...
    NSString *str = [self getMMSSFromSS:value];
//    if (day != 0) {
//        str = [NSString stringWithFormat:@"%d天",day];//%d小时%d分%d秒",day,house,minute,second];
...
-(NSString *)getMMSSFromSS:(NSTimeInterval )seconds{
    ...
    if (day != 0) {
        format_time = [NSString stringWithFormat:@"%d天",day];//%d小时%d分%d秒",day,house,minute,second];
    }else if (day==0 && house != 0) {
        format_time = [NSString stringWithFormat:@"%d小时",house];
    }else if (day==0 && house <= 2) {
        format_time = [NSString stringWithFormat:@"%d小时%d分%d秒",house,minute,second];
    }else if (day== 0 && house== 0 && minute!=0) {
        format_time = [NSString stringWithFormat:@"%d分%d秒",minute,second];
    }else{
        format_time = [NSString stringWithFormat:@"%d秒",second];
    }
    return format_time;
...
-(NSString *)getNowTimeTimestamp{
    ...
    NSString *timeSp = [NSString stringWithFormat:@"%ld", (long)[self getNowTimeStamp]*1]; //秒
